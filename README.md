...
`qishui.js` 的 Android `track_v2` 请求会在发送前刷新 `_rticket`、`X-SS-Req-Ticket` 与全部 X-Headers，并将最终 URL 和 Base64 请求体提交到汽水签名接口实时签名。

旧签名接口 `http://api.music.qishui.vsaa.cn/qm/api.php` 已下线，新版签名服务为 `POST /v1/xheaders/sign`，需携带控制台签发的 `qs_` 开头 API Key。
因此插件新增用户变量：

- `sessionid`：汽水/抖音登录 Cookie 中的 sessionid，可填纯值或包含 `sessionid=...` 的 Cookie 片段。
- `signUrl`：签名服务地址，默认 `http://api.music.qishui.vsaa.cn/v1/xheaders/sign`。
- `signKey`：控制台 API Key；不填时插件会跳过 Android `track_v2` 签名，改用 SEO `url_player_info` 返回的播放地址。

注册签名服务控制台：`http://api.music.qishui.vsaa.cn/console/login.html`，注册后自动获得 API Key 与每日免费配额。
